...
2. Feature subsystems have their own services: `AgentBrowserService` (browser tools), `ComputerUseService` (desktop automation via bundled `agent-desktop`), `SpecialistAgentStore` / `SpecialistAgentRoster` (sidebar Agents roster in Application Support `agents.v1.json`; session binding via `SavedSessionRecord.specialistAgentID`; linked-role sync via `SubagentRoleStore`), `CustomModelStore` / `ProviderStore` (OpenAI-compatible models in `~/.grok/config.toml`), `CursorBridgeRuntime` (managed Cursor OpenAI sidecar — key via `CursorBridgeAPIKey`, not config.toml).
...
Agents roster CRUD lives in the sidebar, not Settings. Settings → Agents manages the default session agent, discovered agents, and custom subagent roles.
