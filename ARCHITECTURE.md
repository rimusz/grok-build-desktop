...
|------|----------|---------|
| **App menu bar** (top of screen) | `AppDelegate.setupMainMenu()` | App (About, Settings ⌘,, Hide ⌘H, Quit), Edit, Project, Session, Window, Help |
...
**Session agent** is also **per session tab** (`SavedSessionRecord.agent`). Each tab launches with its own `--agent` (`ChatStore.effectiveAgentSelection`): an explicit per-tab override when set, otherwise the global default `grokbuild.selectedAgent` (Settings → Agents). The composer shows an **agent/role pill** (`ChatView.agentStatusPill`, first in the composer row, before mode + model — not the Plan/Agent/Auto accept mode control) whose **Run this session as** menu lists the built-in Default option (`GrokAgentProfiles.builtInOptions`), agents discovered for the workspace, and **Custom roles**. `SessionRoleMenu` states that custom roles run the whole session and do not spawn a subagent; roster-linked roles display the Agent name + glyph while unlinked roles keep their raw role name. Picking one calls `ChatStore.setSessionAgent` → **restarts that tab's grok** (agents can only change at launch) and posts `.liveSessionAgentChanged` → `persistSessionLayout()`. The label uses `DashboardTitle.compactRole` (e.g. `Default (grok build)` → `Default`). A tab that has not been overridden follows the global default live (so changing the default and restarting adopts it); overridden tabs keep their choice. Only overridden tabs persist a value (`ChatStore.persistedAgentSelection`).
...
`SavedSessionRecord`: `id`, `workspaceID`, `grokSessionID`, `title`, per-tab `model` / `agent`, optional Agent roster binding `specialistAgentID`, and `lastAccessed`.
...
| `~/Library/Application Support/GrokBuild/agents.v1.json` | Agents roster (`SpecialistAgentStore`) — JSON array of durable identities (name, instructions/`mission`, glyph, color, optional role/model, permission preset, last tab id, **`isPinned`**). Not UserDefaults; not `~/.grok/` |
...
**Sidebar roster.** `SidebarView` shows a global **Agents** section above Projects (then Sessions). Default view is **active only** (`SpecialistAgentRoster.displayed`): pinned (`SpecialistAgent.isPinned` in `agents.v1.json`) or a live session bound in the current project. **Show all agents** (checkbox in the section, `grokbuild.showAllAgents`) reveals the rest. Empty roster offers **Add sample agents** and **New Agent**; empty active view offers **Show all** and **New Agent**. Click focuses a live session in the current project (`SpecialistAgentRoster.liveBinding`: explicit `specialistAgentID` matches, most recently accessed when several; else `lastSessionID`; else matching `--agent` / role not bound to another Agent) or starts a new tab via `createLiveSession` with the Agent’s title, linked role, and `specialistAgentID`. No selected project presents an Add/select-project alert. Context menu: Start session, Open last session (when a bound tab still exists), Pin / Unpin Agent, Edit, Duplicate, Delete. Delete asks for confirmation in `ContentView`; it removes the roster identity and clears session bindings, while linked grok roles and existing sessions stay. Full list still allows create/edit/start. Roster Working vs Idle is true when any bound session in the current project is streaming or needs input. `AgentEditorSheet` writes the store then `syncLinkedRoles` (pin is preserved). `.specialistAgentsChanged` is posted on roster persist.
...
The settings chrome uses a wrapping tab bar (`SettingsView.settingsTabBar` + `SettingsTabFlow`) instead of `TabView`’s compressing segmented control — full titles stay readable and every tab stays visible instead of hiding sideways under the hidden title bar. The Back/Settings header uses `.bar` so it occupies the titlebar the same way the chat toolbar does. Visited panes stay mounted in a `ZStack` (`SettingsTabKeepAlive`) so `@State` / `.task` are not reset when switching tabs. The Agents pane scrolls so the tall custom-roles list cannot clip the chrome.
...
|-----|------|-------------|
| `.agents` | Sidebar-management caption; collapsible discovered-agent list; **default** session-agent picker (new sessions); **custom subagent roles** CRUD + delete confirmation | `listAgents`, `grokbuild.selectedAgent`, `SubagentRoleStore`, `SubagentDeleteCopy` |
