...
- Streaming agent sessions for `grok agent stdio` with Markdown (headings, tables, fenced code, and grok CLI–style colors), thinking blocks, CLI-style working lines (`Read 1 skill, Listed 1 dir  [hooks: 5]`), live tool cards, permission prompts, plan/question cards, and diff review. The agent shell runs in-app via ACP `terminal/*` (create / output / wait / kill / release).
